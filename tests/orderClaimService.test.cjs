...
const digitalDelivered = { ...delivered, urunTipi: "dijital", fizikselKargo: false, dijitalTeslimat: true };
const digitalIssue = { type: "itiraz", reasonCode: "dosya_indirilemiyor", description: "Dosya açılamıyor." };
...
test("buyer dijital koruma süresinde sorun bildirebilir", () => assert.equal(validateClaimRequest(digitalDelivered, buyer, digitalIssue, new Date("2026-09-02T00:00:00Z")).tip, "itiraz"));
test("başka kullanıcı dijital sorun bildiremez", () => assert.throws(() => validateClaimRequest(digitalDelivered, { uid: "other" }, digitalIssue, new Date("2026-09-02T00:00:00Z")), /oluşturamazsınız/));
test("48 saat sonrası yeni dijital claim reddedilir", () => assert.throws(() => validateClaimRequest(digitalDelivered, buyer, digitalIssue, new Date("2026-09-03T00:00:01Z")), /süresi dolmuş/));
...
test("dijital claim seller pending'i blokeler ve finans kaydı üretmez", async () => { const db = fakeFirestore(digitalDelivered); const result = await createClaim({ firestore: db, orderId: "order-1", user: buyer, body: digitalIssue, now: () => new Date("2026-09-02T00:00:00Z") }); const order = db.data.get("siparisler/order-1"); assert.equal(order.hakEdisBlokeli, true); assert.equal(order.aktifTalepId, result.claimId); assert.equal([...db.data.keys()].some((key) => key.startsWith("wallets/") || key.startsWith("refundFinalizations/")), false); });
