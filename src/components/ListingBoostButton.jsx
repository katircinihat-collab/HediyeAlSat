...
export default function ListingBoostButton({ listing, disabled = false }) {
  ...
  return <>
    <button type="button" className="listing-boost-trigger" disabled={disabled} onClick={() => { setError(""); setOpen(true); }}>
    ...
    {open && <div className="listing-boost-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && !loading && setOpen(false)}>
      <section className="listing-boost-dialog" role="dialog" aria-modal="true" aria-labelledby={`boost-title-${listing.id}`}>
        <header><div><small>İLAN GÖRÜNÜRLÜĞÜ</small><h2 id={`boost-title-${listing.id}`}>İlanını öne çıkar</h2><p>{listing.baslik || "İlan"}</p></div><button type="button" aria-label="Pencereyi kapat" disabled={loading} onClick={() => setOpen(false)}>×</button></header>
        ...
        <footer><button type="button" className="secondary" disabled={loading} onClick={() => setOpen(false)}>Vazgeç</button><button type="button" disabled={loading} onClick={startPayment}>{loading ? "Güvenli ödeme hazırlanıyor..." : "Güvenli Ödemeye Geç"}</button></footer>
      </section>
    </div>}
  </>;
}
