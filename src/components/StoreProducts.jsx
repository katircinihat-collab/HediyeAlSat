...
function StoreProducts({
  ...
  const availableListings = useMemo(() => filterAvailableListings(ilanlar), [ilanlar]);

  const kullanilanAnaKategoriler = useMemo(() => {
    const mevcut = new Set(availableListings.map(getListingMainCategory).filter(Boolean));
  ...
  }, [anaKategori, availableListings]);

  const gorunenIlanlar = useMemo(() => {
    const aranan = metinNormallestir(arama);
    const sonuc = availableListings.filter((ilan) => {
