...
function ProductSide({ side, product, disabled, selected, onVote, results, ownerView = false }) {
  ...
  return <article className={`community-battle-side side-${side.toLowerCase()} ${selected ? "is-selected" : ""}`}>
    ...
    {product.available === false ? <button disabled>Bu ürün artık satışta değil</button> : onVote ? <button type="button" disabled={disabled} onClick={() => onVote(side)} aria-label={`${product.title} ürününü seç`}>{selected ? "✓ SENİN OYUN" : "⚔️ Bunu Seç"}</button> : <Link className="community-battle-buy" to={`/ilan/${product.id}`}>{ownerView ? "🔎 Ürünü İncele" : "🎁 Bunu Al"}</Link>}
  </article>;
}
...
export default function CommunityGiftBattleArena({ battle, busy = false, onVote }) {
  ...
  return <section className="community-battle-arena" aria-label="Hediye Kapışması">
    ...
    <div className="community-battle-versus">
      <ProductSide side="A" product={battle.productA} disabled={!canVote || busy} selected={battle.selectedChoice === "A"} onVote={canVote ? onVote : null} results={battle.results} ownerView={battle.isOwner} />
      <strong className="community-battle-vs" aria-label="karşı">VS</strong>
      <ProductSide side="B" product={battle.productB} disabled={!canVote || busy} selected={battle.selectedChoice === "B"} onVote={canVote ? onVote : null} results={battle.results} ownerView={battle.isOwner} />
    </div>
