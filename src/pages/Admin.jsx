...
import AdminGiftBattles from "../components/admin/AdminGiftBattles";
...
{[["dashboard","Dashboard"],["actions","İşlem Gerektirenler"],["orders","Siparişler"],["users","Kullanıcılar"],["listings","İlanlar"],["finance","Finans"],["battles","Kapışmalar"],["raffles","Kura"],["system","Sistem Durumu"],["archive","Arşiv"]].map(([key,label]) => <button type="button" key={key} className={activeSection === key ? "active" : ""} onClick={() => setActiveSection(key)}>{label}</button>)}
</nav>

{activeSection === "dashboard" && <AdminOperationsOverview />}
{activeSection === "system" && <AdminSystemStatus />}
{activeSection === "raffles" && <AdminRaffles />}
{activeSection === "battles" && <AdminGiftBattles />}
