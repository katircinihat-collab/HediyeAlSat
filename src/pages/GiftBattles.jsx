...
export default function GiftBattles() {
  ...
  useEffect(() => {
    ...
    getCommunityBattles().then((data) => { if (active) setFeed(data.battles || []); })
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true); setError("");
    getCommunityBattle(battleId).then((data) => { if (active) setBattle(data.battle); })
      .catch((e) => active && setError(e.message))
      .finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [battleId, user]);
  ...
  if (loading) return <main className="community-battle-page"><header className="community-battle-feed-head"><span>KEŞFET · OYLA · KARAR VER</span><h1>⚔️ Hediye Kapışmaları</h1><p>Topluluğun kararsız kaldığı hediyelere fikrini söyle.</p></header><section className="community-battle-loading" aria-label="Kapışmalar yükleniyor" aria-busy="true"><span /><span /><span /><span /></section></main>;
